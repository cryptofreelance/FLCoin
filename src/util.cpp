...
const char * const BITCOIN_CONF_FILENAME = "flcoin.conf";
const char * const BITCOIN_PID_FILENAME = "flcoind.pid";
 ...
 * by well-meaning flcoin adding mutexes in the most straightforward way.
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "flcoin" is a composite category enabling all FLCoin-related debug output
            if(ptrCategory->count(string("flcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FLCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FLCoinCore
    // Mac: ~/Library/Application Support/FLCoinCore
    // Unix: ~/.flcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FLCoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/FLCoinCore";
#else
    // Unix
    return pathRet / ".flcoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty flcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override flcoin.conf

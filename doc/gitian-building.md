...
*Setup instructions for a Gitian build of FLCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the FLCoin
...
- [Building FLCoin Core](#building-flcoin-core)
...
Clone the git repositories for FLCoin Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/flcoinproject/flcoin
```
...
in the FLCoin Core repository under 'Fetch and build inputs' to install sources which require
...
To build FLCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the FLCoin Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/flcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/flcoinproject/flcoin
...
For example:
```bash
URL=https://github.com/crowning-/flcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit flcoin=${COMMIT} --url flcoin=${URL} ../flcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit flcoin=${COMMIT} --url flcoin=${URL} ../flcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit flcoin=${COMMIT} --url flcoin=${URL} ../flcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the flcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../flcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/flcoin.git
SIGPATH=/some/root/path/flcoin-detached-sigs.git

./bin/gbuild --url flcoin=${BTCPATH},signature=${SIGPATH} ../flcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/flcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/flcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/flcoin-osx-build.assert
```
...
[flcoin/gitian.sigs](https://github.com/flcoinproject/gitian.sigs/) repository, or if that's not possible to create a pull
request.
